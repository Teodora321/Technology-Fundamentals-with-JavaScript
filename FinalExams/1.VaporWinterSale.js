    ...
    for(let token of tokens){
        if(token.includes('-')){
            ...
            if(games.hasOwnProperty(name)){
                games[name].dlc = dlc
                games[name].price*=1.20;
            }
        }
    }
    for(let game in games){
        if(games[game].hasOwnProperty('dlc')){
            games[game].price*=0.50;
        }else{
...
systemRegister([ 'WitHer 3-50, FullLife 3-60, WitHer 3:Blood and Beer, Cyberfunk 2077-120, League of Leg Ends-10, League of Leg Ends:DoaT' ])